...
selected SearchResult
  ...
  → start_sec = max(0, floor(start_at_ms / 1000))
...
- 기본 간격은 5초다.
- `get_transcription()`은 네트워크 오류, 429, 5xx를 `RetryableError`로, 인증·검증·4xx를 `FatalError`로 구분한다.
- 폴링 루프는 `RetryableError`만 백오프 후 동일 ID로 재시도하고, 그 외 오류는 즉시 중단한다.
...
오류 메시지는 자격 증명, 토큰, 원본 요청 헤더와 전체 API 응답을 포함하지 않는다. RTZR 응답 본문 전체를 로그에 기록하지 않는다.
