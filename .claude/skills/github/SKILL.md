...
1. Inspect the diff and commit list against the selected base branch.
2. Resolve a body template in this order:
   - A matching template under `.github/PULL_REQUEST_TEMPLATE/`, `PULL_REQUEST_TEMPLATE/`, or `docs/PULL_REQUEST_TEMPLATE/`
   - `.github/pull_request_template.md`, `pull_request_template.md`, or `docs/pull_request_template.md`
   - `assets/pr-template.md` from this skill
3. If multiple repository templates plausibly match, ask the user which one to use.
4. Fill the template only with verified information. Remove comments, empty optional sections, and unused placeholders.
5. State the exact test commands and results. If tests were not run, say so and explain why.
6. Write the completed body to a temporary Markdown file and use `--body-file`.
...
Use the `github-issue-creator` skill to select and fill the issue template. Publish its completed Markdown file with `--body-file`; do not collapse multiline content into a shell argument.
